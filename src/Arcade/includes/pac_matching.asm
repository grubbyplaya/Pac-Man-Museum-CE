...
; matching Pac-Man disasm

#define DIPSwitch $50C0

#include "includes/pacdefines.asm"
...
RST_38:         ; $0038 - waitloop
    ; TODO - IM 2 doesn't work on the CE. Fix that!
    xor     a
    ld      ($5000), a
    ld      ($5007), a
    jp      RST_38
...
_:  ld      a, ($4DA6)
    ...
    ; TODO - check for green-eyed ghost bug
...
LABEL_413:
    ...
    .dw QuickRet      ; returns immediately
    .dw LABEL_4A5  ; draw blue ghost (inky)
    .dw QuickRet      ; returns immediately
...
_:  ld      ix, DATA_796
    ...
    ld      e, a
    ld      d, 0    
    ; TODO: easy mlt replacement here
    add     ix, de
    ...
    add     a, c 
    add     a, b    ; A = A*42
    ; TODO: even easier mlt replacement here
    ld      e, a
    ld      d, 0
    ld      hl, DATA_330F
    add     hl, de
    ...
    ld      e, a
    ld      d, 0
    ; TODO: somewhat easy mlt replacement here
    ld      hl, DATA_843
    add     hl, de
    call    LABEL_83A
...
_:  ld      (hl), a
    ...
    ; do an interrupt
    ; TODO: fix interrupt (broken on 84+ CE)
    im      2
    ld      a, $FA
    out     (0), a
...
Patch1C = $+1   ; Ms. Pac - Patch 28
    ...
    ; TODO: It's slow as hell to redraw the whole tilemap
    ; to change one color. Just do palette cycling on 84+ CE
...
; init, check checksums
LABEL_3000:
    ; TODO: 84+ CE doesn't need self-tests. delete everything from here til LABEL_30C1
    ld      hl, $0000
LABEL_3003:
    ld      bc, $1000
LABEL_3006:
...
_:  ld      a, c
    ...
    ; test if the written values remain
LABEL_3072:
    ld      ($50C0), a
...
LABEL_3188:
    ...
    ; TODO: unecessary 84+ CE code starts here
    ld      a, ($5000)
    cpl
    ld      b, a
    and     $E0
...
_:  ld      a, b
    ...
    ld      a, $20
    ld      ($4EBC), a
    ; TODO: unecessary 84+ CE code ends here
...
_:  call    LABEL_32ED
    ...
    ; loop until service switch is off
_:  ld      ($50C0), a
    ld      a, ($5040)
    and     $10
    jr      z, -_

    ; TODO: if you want to remove all the glitchy-looking tiles
    ; and stuff, delete everything from LABEL_3188 to here
    ld      a, ($5040)
    and     $60
    jp      nz, LABEL_234B
...
DATA_3435:
    #import "includes/mazedata.bin"

DATA_35B5:
    #import "includes/pelletdata.bin"
...
Data_3BD4:
#import "sounds/song_3BD4.bin"

DATA_3BF3:
#import "sounds/song_3BF3.bin"

DATA_3C58:
#import "sounds/song_3C58.bin"

DATA_3C95:
#import "sounds/song_3C95.bin"
...
DATA_3CE0:  ; zerofill
.fill $3D00 - DATA_3CE0, $00

#include "includes/strings2.asm"

DATA_3E5C:  ; zerofill
.fill $3FFA - DATA_3E5C, 0
